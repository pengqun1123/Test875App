package com.arcFace.util.camera;
...
//                List<Camera.Size> supportedPreviewSize = getCommonSupportedPreviewSize();
                ...
                if (!isSetPreView) {
                    Logger.d("最好  相机设置预览参数:" + width + "   " + height);
    ...
    private Camera.Size getBestSupportedSize(List<Camera.Size> sizes, Point previewViewSize) {
        ...
        for (Camera.Size s : sizes) {
            ...
            if (isNormalRotate) {
                if (Math.abs((s.height / (float) s.width) - previewViewRatio) < Math.abs(bestSize.height
                        / (float) bestSize.width - previewViewRatio)) {
                    bestSize = s;
                    Logger.d("最好：  111 w:" + s.width + "   h:" + s.height);
                }
            } else {
                if (Math.abs((s.width / (float) s.height) - previewViewRatio) < Math.abs(bestSize.width
                        / (float) bestSize.height - previewViewRatio)) {
                    bestSize = s;
                    Logger.d("最好：  222 w:" + s.width + "   h:" + s.height);
                }
            }
        }
        int height = bestSize.height;
        int width = bestSize.width;
        Log.d("最好：", "  height:" + height);
        Log.d("最好：", "  width:" + width);
        return bestSize;
    }
